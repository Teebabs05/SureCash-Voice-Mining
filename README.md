...
- **Payments:** pluggable gateway interface — Paystack fully wired; Monnify and Korapay wired but best-effort (see Known limitations); PayVessel wired for withdrawal payouts only (its deposit product is virtual accounts, a different modality, not wired up); Flutterwave a stub to fill in with real credentials
...
- `FLUTTERWAVE_SECRET_KEY` — stub adapter ready for real credentials
...
- **Deposits** — manual bank transfer with receipt upload (admin-approved), instant gateway checkout (Paystack live; Monnify/Korapay wired but best-effort; Flutterwave stubbed — PayVessel isn't offered here, see Withdrawals), or a dedicated BillStack virtual account number (auto-credited via webhook on any transfer in — best-effort integration, see Known limitations).
...
- **Payment gateways**: Paystack is a real, confirmed integration (deposits via `src/lib/payments/provider.ts`, instant withdrawal payouts via `src/lib/payments/payout-provider.ts`); Flutterwave throws a clear "not configured" error until real API credentials are wired in, and automatic withdrawals simply fall back to the manual admin flow in that case.
